...
export function SurveyRunner({ slug, googleEnabled, notice }: { slug: string; googleEnabled: boolean; notice?: string }) {
  ...
  if (stage === "loading") return <div className="survey-card" role="status"><h2>Preparando tu espacio…</h2><p>Cargando la encuesta de Psico Care.</p></div>;
  if (stage === "unavailable" || !survey) return <div className="survey-card"><h2>La encuesta no está disponible ahora.</h2><p role="alert">{error}</p><button className="button button-primary" onClick={() => { setError(""); setStage("loading"); setReload(r => r + 1); }}>Volver a intentar</button><p>Puedes seguir explorando <Link href="/#app-movil" className="text-action">la demo de Psico Care</Link>.</p></div>;
  const question = survey.questions[index];
  return <div className="survey-card" ref={panel} tabIndex={-1} aria-busy={busy}>
    {stage === "intro" && <><h2>Antes de comenzar, tú decides.</h2><p>Esta investigación nos ayuda a priorizar funciones y mejorar Psico Care. No es una evaluación de tu salud.</p>
    ...
    {stage === "success" && <div className="survey-success"><Image src="/brand/mascot-calm.webp" width={145} height={216} alt="El personaje tranquilo de Psico Care agradece tu participación"/><h2>{deleted ? "Tu participación fue retirada." : "Gracias por ayudarnos a construir Psico Care."}</h2><p role="status">{deleted ? "Se eliminaron las respuestas asociadas a este navegador." : "Tus respuestas quedaron guardadas. Nos ayudarán a construir herramientas que tengan sentido para quienes las usarán."}</p>{!deleted && <EmailOptIn slug={slug} googleEnabled={googleEnabled} participant={participant} notice={notice} onChange={setParticipant}/>}<div className="button-row"><ButtonLink href="/#app-movil">Volver a explorar la app</ButtonLink></div></div>}
