...
export function Contact() {
  ...
  return <section className="contact-section section-space" id="contacto"><Container><div className="contact-layout">
    ...
    {status === "success" ? <div className="contact-success" role="status"><Icon name="check" size={40}/><h3>Gracias por acercarte.</h3><p>Tu mensaje quedó guardado en el buzón de Psico Care para que el equipo pueda revisarlo.</p><button className="text-action" onClick={() => { setStatus("idle"); requestId.current = ""; }}>Escribir otro mensaje<Icon name="arrow" size={17}/></button></div> :
    <form className="contact-form" onSubmit={async event => {
      ...
      <label>¿Cómo te gustaría participar?<textarea name="message" required minLength={10} maxLength={2000} rows={4} placeholder="Cuéntanos sobre tu institución y tu interés en Psico Care. Evita incluir información de salud personal."/></label>
